<template>
  ...
  <div class="col-lg-8 my-5">
    ...
    <div v-else class="card mb-4">
      <h4 v-if="post" class="m-4">{{ post.title }}</h4>
      <div v-if="post" class="m-4" v-html="post.content"></div>
      <p class="mx-4">
        <em>
          Published by:
          <a v-if="post" href="#">{{ post.author.firstName }}</a>
        </em>
      </p>
    </div>
    <!-- Author -->
    ...
    <ul v-if="!isFetching && postComments" class="list-unstyled mx-2">
    ...
    <ul v-else class="list-unstyled mx-2">
      <li>
        <hr class="dashed" />
        <small-loader />
      </li>
...
export default {
  ...
  computed: {
    ...
    getUrl() {
      ...
      return `${process.env.APP_BASE_URL}/posts/${id}`
    }
  },
  beforeCreate() {
    const { id } = this.$route.params
